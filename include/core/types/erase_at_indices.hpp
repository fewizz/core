...
namespace types {
	...
	class erase_at_indices {
		...
		struct index_and_types<Index, HeadType, TailTypes...> {
			...
			struct resulting<ResultingTypes...> {
				using types = typename index_and_types<Index + 1, TailTypes...>::template resulting<ResultingTypes...>::types;
			};

			template<typename... ResultingTypes>
			requires(!skip_index)
			struct resulting<ResultingTypes...> {
				using types = typename index_and_types<Index + 1, TailTypes...>::template resulting<ResultingTypes..., HeadType>::types;
			};
		};
	...
	public:
		...
		using for_types_of = typename index_and_types<0, Types...>::template resulting<>::types;
