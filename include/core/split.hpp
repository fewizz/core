...
#include "range.hpp"
#include "range_of_value_type_same_as.hpp"
...
struct splitted_iterator {
		...
		split_range{ split_range }
	{
		while(
			...
			contains(*begin).in(split_range)
		) { ++begin; }
		end = begin;
		if(end != base_iterator_end) ++end;
		while(
			end != base_iterator_end &&
			!contains(*end).in(split_range)
		) { ++end; }
	}
...
template<range Range, range_of<value_type<Range>> SplitRange>
struct splitted {
	Range& base_range;
	SplitRange split_range;
...
struct split {
	...
	template<range_of<value_type<Range>> SplitRange>
