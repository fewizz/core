...
struct of<> {
	...
	template<class T>
	using append_back_types = of<T>;

	template<class T>
	using append_front_types = of<T>;
	...
	using indices_of_types_invocable_with = indices::of<>;

	template<typename... Args>
	using indices_of_types_not_invocable_with = indices::of<>;
};
...
struct of {
	...
	using type_at_index = typename ::at<Index>::template of_types<Ts...>;
...
public:
	template<typename T>
	using types_at = types_at_t<T>;

	using front = type_at_index<0>;
	using back = type_at_index<size - 1>;

	template<typename... Ts0>
	using append_back_types = of<Ts..., Ts0...>;

	template<typename... Ts0>
	using append_front_types = of<Ts0..., Ts...>;
	...
	template<typename... Args>
	using indices_of_types_invocable_with =
		indices_of_types_that_satisfy<
			are::types<Args...>::template args_for_predicate
		>;

	template<typename... Args>
	using indices_of_types_not_invocable_with =
		indices_of_types_that_not_satisfy<
			are::types<Args...>::template args_for_predicate
		>;
	...
	private:
	template<typename T>
	struct erase_type_at_t;

	template<std::size_t... Indices>
	struct erase_type_at_t<indices::of<Indices...>> {
		using type =
			types_at<
...
public:
	...
	template<typename T>
	using erase_at = typename erase_type_at_t<T>::type;

	template<typename T>
	static constexpr std::size_t index_of_first_type = indices_of_types_same_as<T>::template value_at<0>;
