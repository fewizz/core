...
struct splitted_iterator {
	...
	constexpr auto operator * () const {
		return iterators::to_range(begin, end);
	}
};
...
constexpr inline bool operator == (
	...
	range::default_sentinel
) {
	return it.begin == it.base_iterator_end;
}

template<basic_range Range, range_of<range::value_type<Range>> SplitRange>
struct splitted {
	Range& base_range;
	SplitRange split_range;
	...
	constexpr auto end() { return range::default_sentinel{}; }
...
struct split {
	...
	template<range_of<range::value_type<Range>> SplitRange>
